...
function ColGroup(props: ColGroupType) {
  const { prefixCls, flattenColumns } = props;

  return (
    <colgroup>
      {flattenColumns.map((col) => {
        if (col.title === INTERNAL_EXPAND_KEY) {
          return <col key={INTERNAL_EXPAND_KEY} className={`${prefixCls}-expand-icon-col`} style={fixedWidth(col.width)} />;
        }
        if (col.title === INTERNAL_SELECTION_KEY) {
          return <col key={INTERNAL_SELECTION_KEY} className={`${prefixCls}-selection-col`} style={fixedWidth(col.width)} />;
        }

        return <col key={col.key} style={fixedWidth(col.width)} />;
      })}
    </colgroup>
  );
}
