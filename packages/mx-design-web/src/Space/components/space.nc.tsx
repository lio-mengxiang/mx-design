...
function NcSpace(baseProps: SpaceProps, ref) {
  const { getPrefixCls, componentConfig } = useContext(ConfigContext);
  const props = useMergeProps<SpaceProps>(baseProps, defaultProps, componentConfig?.Space);
  const { className, style, children, size, direction, align, split, ...rest } = props;
...
const SpaceComponent = forwardRef<unknown, SpaceProps>(NcSpace);

SpaceComponent.displayName = 'Space';

export { SpaceComponent as NcSpace };
